  ...
  return (
    <AppShell>
      <Section className="max-w-2xl">
        <AgentOnboardingSteps current="email" />
        <Card>
          ...
          {!verified ? (
            <div className="mt-6 grid gap-3">
              <form action="/api/agent/email-verification" method="post" className="grid gap-3">
                <input type="hidden" name="email" value={email} />
                <label className="grid gap-1.5 text-sm font-medium text-slate-700">
                  Verification code
                  <input name="code" placeholder="123456" className="min-h-11 rounded-md border border-slate-300 px-3" />
                </label>
                <button className="min-h-11 rounded-md bg-teal-700 px-4 text-sm font-semibold text-white hover:bg-teal-800">
                  Verify email
                </button>
              </form>
              <form action="/api/agent/email-verification" method="post">
                <input type="hidden" name="email" value={email} />
