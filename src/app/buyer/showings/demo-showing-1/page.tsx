      ...
      <Section className="max-w-4xl">
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-2">
          ...
          <Card>
            ...
            <dl className="mt-4 grid gap-3 text-sm">
              <div><dt className="text-slate-500">Payment</dt><dd>{showing.paymentStatus} - {formatMoney(showing.showingFeeCents)}</dd></div>
              <div><dt className="text-slate-500">Assigned agent</dt><dd>{agent?.name ?? "Waiting for first accept"}</dd></div>
              <div><dt className="text-slate-500">Agents notified</dt><dd>{matches.map((match) => match.name).join(", ")}</dd></div>
            </dl>
          </Card>
        </div>
        <div className="mt-6">
          <ButtonLink href="/buyer/dashboard" variant="secondary">Back to dashboard</ButtonLink>
        </div>
      </Section>
    </AppShell>
  );
